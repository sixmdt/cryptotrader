...
def unite_pacs():
    ...
    for i in range(1, len(mid_pac)):
        if (last_main_trade.get('date') < mid_pac[-i].get('date')) and (last_main_trade.get('trade_id') != mid_pac[-i].get('trade_id')):
...
last_main_trade = main_trades[-1]

while True:
    time.sleep(60)
    mid_pac = get_trades(500)

    if mid_pac.index(last_main_trade) < 499:
        unite_pacs()
        
        print('{} \n'.format(len(main_trades)))
        
        df = pd.DataFrame(main_trades)
        df.to_csv('trades_data', sep=',', mode='a', header=None, index=None)

        last_main_trade = main_trades[-1]
        main_trades.clear()
    else:
        print('--!-- >499 --!-- \n', mid_pac.index(last_main_trade))